------------------------------------------------------------------------
-- Syntax of pure type systems with subtyping (PTS-≤)
------------------------------------------------------------------------

module PtsSub.Syntax where
...
module _ (Sort : Set) where
  ...
  module Syntax where
    ...
    data Term (n : ℕ) : Set where
      ...
      Π     : (a : Term n) (b : Term (1 + n))      → Term n  -- product
      ƛ     : (a : Term n) (b : Term (1 + n))      → Term n  -- abstraction
      _·_   : (a : Term n) (b : Term n)            → Term n  -- application
      _…_∈_ : (a : Term n) (b : Term n) (s : Sort) → Term n  -- interval
      ⟨_⟩   : (a : Term n)                         → Term n  -- singleton
      _!    : (a : Term n)                         → Term n  -- projection
  ...
  module SubstApp {T} (l : Lift T Term) where
    ...
    -- Substitutions in intervals are compositional.
    …-/✶-↑✶ : ∀ k {m n a b s} (σs : Subs T m n) →
              a … b ∈ s /✶ σs ↑✶ k ≡ (a /✶ σs ↑✶ k) … (b /✶ σs ↑✶ k) ∈ s
    …-/✶-↑✶ k ε        = refl
    …-/✶-↑✶ k (σ ◅ σs) = cong₂ _/_ (…-/✶-↑✶ k σs) refl

    -- Substitutions in singletons are compositional.
    ⟨⟩-/✶-↑✶ : ∀ k {m n a} (σs : Subs T m n) →
               ⟨ a ⟩ /✶ σs ↑✶ k ≡ ⟨ a /✶ σs ↑✶ k ⟩
    ⟨⟩-/✶-↑✶ k ε        = refl
    ⟨⟩-/✶-↑✶ k (σ ◅ σs) = cong₂ _/_ (⟨⟩-/✶-↑✶ k σs) refl

    -- Substitutions in projections are compositional.
    !-/✶-↑✶ : ∀ k {m n a} (σs : Subs T m n) →
              a ! /✶ σs ↑✶ k ≡ (a /✶ σs ↑✶ k) !
    !-/✶-↑✶ k ε        = refl
    !-/✶-↑✶ k (σ ◅ σs) = cong₂ _/_ (!-/✶-↑✶ k σs) refl
  ...
  module Substitution where
    ...
    module Lemmas {T₁ T₂} {lift₁ : Lift T₁ Term} {lift₂ : Lift T₂ Term} where
      ...
      /✶-↑✶ σs₁ σs₂ hyp k (a · b) = begin
        ...
        ≡⟨ sym (·-/✶-↑✶ _ k σs₂) ⟩
          (a · b) /✶₂ σs₂ ↑✶₂ k
        ∎
      /✶-↑✶ σs₁ σs₂ hyp k (a … b ∈ s) = begin
          (a … b ∈ s) /✶₁ σs₁ ↑✶₁ k
        ≡⟨ …-/✶-↑✶ _ k σs₁ ⟩
          (a /✶₁ σs₁ ↑✶₁ k) … (b /✶₁ σs₁ ↑✶₁ k) ∈ s
        ≡⟨ cong₂ (λ a b → a … b ∈ _) (/✶-↑✶ σs₁ σs₂ hyp k a)
                                     (/✶-↑✶ σs₁ σs₂ hyp k b) ⟩
          (a /✶₂ σs₂ ↑✶₂ k) … (b /✶₂ σs₂ ↑✶₂ k) ∈ s
        ≡⟨ sym (…-/✶-↑✶ _ k σs₂) ⟩
          (a … b ∈ s) /✶₂ σs₂ ↑✶₂ k
    ...
    open TermLikeLemmas termLikeLemmas public
      using (varLiftAppLemmas; varLiftSubLemmas; _/α_)
