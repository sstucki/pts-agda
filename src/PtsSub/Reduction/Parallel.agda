------------------------------------------------------------------------
-- Parallel reduction in pure type systems with subtyping (PTS-≤)
------------------------------------------------------------------------

module PtsSub.Reduction.Parallel where
...
module _ {Sort : Set} where
  ...
  data _⇛_ {n : ℕ} : Term n → Term n → Set where
    ...
    ƛ·   : ∀ {a b₁ b₂ c₁ c₂} → b₁ ⇛ b₂ → c₁ ⇛ c₂ →
           (ƛ a b₁) · c₁ ⇛ b₂ [ c₂ ]
    _…_  : ∀ {a₁ a₂ b₁ b₂ s} → a₁ ⇛ a₂ → b₁ ⇛ b₂ → a₁ … b₁ ∈ s ⇛ a₂ … b₂ ∈ s
    ⟨_⟩  : ∀ {a₁ a₂}         → a₁ ⇛ a₂           →      ⟨ a₁ ⟩ ⇛ ⟨ a₂ ⟩
    _!   : ∀ {a₁ a₂}         → a₁ ⇛ a₂           →        a₁ ! ⇛ a₂ !
    ⟨_⟩! : ∀ {a₁ a₂}         → a₁ ⇛ a₂           →    ⟨ a₁ ⟩ ! ⇛ a₂
  ...
  …-⇛ : ∀ {n} {a : Term n} {b c s} → a … b ∈ s ⇛ c →
        ∃ λ a′ → ∃ λ b′ → a ⇛ a′ × b ⇛ b′ × a′ … b′ ∈ s ≡ c
  …-⇛ refl          = _ , _ , refl , refl , P.refl
  …-⇛ (a⇛a′ … b⇛b′) = _ , _ , a⇛a′ , b⇛b′ , P.refl
  ...
  ƛ-⇛* : ∀ {n} {a : Term n} {b c} → ƛ a b ⇛* c →
         ∃ λ a′ → ∃ λ b′ → a ⇛* a′ × b ⇛* b′ × ƛ a′ b′ ≡ c
  ƛ-⇛* ε                        = _ , _ , ε , ε , P.refl
  ƛ-⇛* (refl        ◅ λab⇛*c)   = ƛ-⇛* λab⇛*c
  ƛ-⇛* (ƛ a⇛a₁ b⇛b₁ ◅ λa₁b₁⇛*c) =
    let a₂ , b₂ , a₁⇛*a₂ , b₁⇛*b₂ , λa₂b₂≡c = ƛ-⇛* λa₁b₁⇛*c
    in a₂ , b₂ , a⇛a₁ ◅ a₁⇛*a₂ , b⇛b₁ ◅ b₁⇛*b₂ , λa₂b₂≡c

  Π-⇛* : ∀ {n} {a : Term n} {b c} → Π a b ⇛* c →
         ∃ λ a′ → ∃ λ b′ → a ⇛* a′ × b ⇛* b′ × Π a′ b′ ≡ c
  Π-⇛* ε                        = _ , _ , ε , ε , P.refl
  Π-⇛* (refl        ◅ Πab⇛*c)   = Π-⇛* Πab⇛*c
  Π-⇛* (Π a⇛a₁ b⇛b₁ ◅ Πa₁b₁⇛*c) =
    let a₂ , b₂ , a₁⇛*a₂ , b₁⇛*b₂ , Πa₂b₂≡c = Π-⇛* Πa₁b₁⇛*c
    in a₂ , b₂ , a⇛a₁ ◅ a₁⇛*a₂ , b⇛b₁ ◅ b₁⇛*b₂ , Πa₂b₂≡c
  ...
  …-⇛* (refl        ◅ a…b⇛*c)   = …-⇛* a…b⇛*c
  …-⇛* (a⇛a₁ … b⇛b₁ ◅ a₁…b₁⇛*c) =
    let a₂ , b₂ , a₁⇛*a₂ , b₁⇛*b₂ , a₂…b₂≡c = …-⇛* a₁…b₁⇛*c
    in a₂ , b₂ , a⇛a₁ ◅ a₁⇛*a₂ , b⇛b₁ ◅ b₁⇛*b₂ , a₂…b₂≡c
  ...
  module ParSubstitution where
    ...
    _/⇛α_ : ∀ {m n a₁ a₂} {σ₁ σ₂ : Sub Fin m n} →
            a₁ ⇛ a₂ → σ₁ V.⟨≡⟩ σ₂ → (a₁ S./Var σ₁) ⇛ (a₂ S./Var σ₂)
    _/⇛α_ = ParSubstApp._/⇛_ varSubstApp

    simple : RelSimple _⇛_ S.simple S.simple
    simple = record
      { extension = record { weaken = λ t₁⇛t₂ → t₁⇛t₂ /⇛α V.wk }
  ...
  refl · a₂⇛b₁₂ ⋄ ƛ· {b₂ = b₂₁} a₁⇛b₂₁ a₂⇛b₂₂ =
    let c₂ , b₁₂⇛c₂ , b₂₂⇛c₂ = a₂⇛b₁₂ ⋄ a₂⇛b₂₂
    in b₂₁ [ c₂ ] , ƛ· a₁⇛b₂₁ b₁₂⇛c₂ , refl {a = b₂₁} [⇛ b₂₂⇛c₂ ]
  ƛ _ a₁⇛b₁₁ · a₂⇛b₁₂ ⋄ ƛ· a₁⇛b₂₁ a₂⇛b₂₂ =
    let c₁ , b₁₁⇛c₁ , b₂₁⇛c₁ = a₁⇛b₁₁ ⋄ a₁⇛b₂₁
        c₂ , b₁₂⇛c₂ , b₂₂⇛c₂ = a₂⇛b₁₂ ⋄ a₂⇛b₂₂
    in c₁ [ c₂ ] , ƛ· b₁₁⇛c₁ b₁₂⇛c₂ , b₂₁⇛c₁ [⇛ b₂₂⇛c₂ ]
  ƛ· {b₂ = b₁₁} a₁⇛b₁₁ a₂⇛b₁₂ ⋄ refl · a₂⇛b₂₂ =
    let c₂ , b₁₂⇛c₂ , b₂₂⇛c₂ = a₂⇛b₁₂ ⋄ a₂⇛b₂₂
    in b₁₁ [ c₂ ] , refl {a = b₁₁} [⇛ b₁₂⇛c₂ ] , ƛ· a₁⇛b₁₁ b₂₂⇛c₂
  ƛ· a₁⇛b₁₁ a₂⇛b₁₂ ⋄ ƛ _ a₁⇛b₂₁ · a₂⇛b₂₂ =
    let c₁ , b₁₁⇛c₁ , b₂₁⇛c₁ = a₁⇛b₁₁ ⋄ a₁⇛b₂₁
        c₂ , b₁₂⇛c₂ , b₂₂⇛c₂ = a₂⇛b₁₂ ⋄ a₂⇛b₂₂
    in c₁ [ c₂ ] , b₁₁⇛c₁ [⇛ b₁₂⇛c₂ ] , ƛ· b₂₁⇛c₁ b₂₂⇛c₂
  ƛ· a₁⇛b₁₁ a₂⇛b₁₂ ⋄ ƛ· a₁⇛b₂₁ a₂⇛b₂₂ =
    let c₁ , b₁₁⇛c₁ , b₂₁⇛c₁ = a₁⇛b₁₁ ⋄ a₁⇛b₂₁
        c₂ , b₁₂⇛c₂ , b₂₂⇛c₂ = a₂⇛b₁₂ ⋄ a₂⇛b₂₂
    in c₁ [ c₂ ] , b₁₁⇛c₁ [⇛ b₁₂⇛c₂ ] , b₂₁⇛c₁ [⇛ b₂₂⇛c₂ ]
  ...
  -- …-injectivity (with respect to ≡p).
  …-inj : ∀ {n} {a₁ a₂ : Term n} {b₁ b₂ s₁ s₂} →
          a₁ … b₁ ∈ s₁ ≡p a₂ … b₂ ∈ s₂ → a₁ ≡p a₂ × b₁ ≡p b₂ × s₁ ≡ s₂
  …-inj a₁…b₁≡a₂…b₂ with ≡p⇒⇛* a₁…b₁≡a₂…b₂
  …-inj a₁…b₁≡a₂…b₂ | c , a₁…b₁⇛*c , a₂…b₂⇛*c with
    …-⇛* a₁…b₁⇛*c | …-⇛* a₂…b₂⇛*c
  …-inj a₁…b₁≡a₂…b₂ | a₃ … b₃ ∈ s , a₁…b₁⇛*c , a₂…b₂⇛*c |
    ._ , ._ , a₁⇛*a₃ , b₁⇛*b₃ , P.refl | ._ , ._ , a₂⇛*a₃ , b₂⇛*b₃ , P.refl =
    ⇛*⇒≡p a₁⇛*a₃ ◅◅ sym (⇛*⇒≡p a₂⇛*a₃) , ⇛*⇒≡p b₁⇛*b₃ ◅◅ sym (⇛*⇒≡p b₂⇛*b₃) ,
      P.refl
