------------------------------------------------------------------------
-- Full β-reduction in pure type systems with subtyping (PTS-≤)
------------------------------------------------------------------------

module PtsSub.Reduction.Full where
...
open import PtsSub.Reduction.Parallel as Par
  hiding (reduction; _⋄*_; Π-inj; …-inj)
...
module _ {Sort : Set} where
  ...
  data _→β_ {n} : Term n → Term n → Set where
    ...
    ƛ·   : ∀ a b c                      → (ƛ a b) · c →β b [ c ]
    _…₁_ : ∀ {a₁ a₂ s} → a₁ →β a₂ → ∀ b →  a₁ … b ∈ s →β a₂ … b ∈ s
    _…₂_ : ∀ {b₁ b₂ s} a → b₁ →β b₂     →  a … b₁ ∈ s →β a … b₂ ∈ s
    ⟨_⟩  : ∀ {a₁ a₂} → a₁ →β a₂         →      ⟨ a₁ ⟩ →β ⟨ a₂ ⟩
    _!   : ∀ {a₁ a₂} → a₁ →β a₂         →        a₁ ! →β a₂ !
    ⟨_⟩! : ∀ a                          →     ⟨ a ⟩ ! →β a
  ...
  record BetaSubstApp {T} (l : Lift T Term) : Set where
    ...
    ƛ· a b c   /→β σ =
      subst (_→β_ _) (sym (/-sub-↑ b c σ)) (ƛ· (a / σ) (b / σ ↑) (c / σ))
    a₁→a₂ …₁ b /→β σ = (a₁→a₂ /→β σ) …₁ (b / σ)
    a …₂ b₁→b₂ /→β σ = (a / σ) …₂ (b₁→b₂ /→β σ)
    ⟨ a₁→a₂ ⟩  /→β σ = ⟨ a₁→a₂ /→β σ ⟩
    a₁→a₂ !    /→β σ = (a₁→a₂ /→β σ) !
  ...
  module BetaSubstitution where
    ...
    open BetaSubstApp varSubstApp public using ()
      renaming (_/→β_ to _/→βα_; _/→β*_ to _/→β*α_; _/≡β_ to _/≡βα_)

    -- Weakening of one-step β-reductions.
    weaken-→β : ∀ {n} {a b : Term n} → a →β b → weaken a →β weaken b
    weaken-→β a→b = a→b /→βα VarSubst.wk
  ...
  ⇛⇒→β* (_·_ {a₁} {a₂} {b₁} {b₂} a₁⇛a₂ b₁⇛b₂) = begin
    ...
    a₂ · b₂   ∎
  ⇛⇒→β* (ƛ· {a} {b₁} {b₂} {c₁} {c₂} b₁⇛b₂ c₁⇛c₂) = begin
    (ƛ a b₁) · c₁   ⟶⋆⟨ gmap (λ b → (ƛ _ b) · _)
                             (λ b₁→b₂ → (ƛ₂ _ b₁→b₂) ·₁ _) (⇛⇒→β* b₁⇛b₂) ⟩
    (ƛ a b₂) · c₁   ⟶⋆⟨ gmap (_·_ _) (_·₂_ _) (⇛⇒→β* c₁⇛c₂) ⟩
    (ƛ a b₂) · c₂   ⟶⟨ ƛ· a b₂ c₂ ⟩
    b₂ [ c₂ ]       ∎
  ⇛⇒→β* (_…_ {a₁} {a₂} {b₁} {b₂} {s} a₁⇛a₂ b₁⇛b₂) = begin
    a₁ … b₁ ∈ s   ⟶⋆⟨ gmap (λ a → a … b₁ ∈ s) (λ a₁→a₂ → a₁→a₂ …₁ _)
                           (⇛⇒→β* a₁⇛a₂) ⟩
    a₂ … b₁ ∈ s   ⟶⋆⟨ gmap (λ b → a₂ … b ∈ s) (_…₂_ _) (⇛⇒→β* b₁⇛b₂) ⟩
    a₂ … b₂ ∈ s   ∎
  ⇛⇒→β* ⟨ a₁⇛a₂ ⟩  = gmap ⟨_⟩ ⟨_⟩ (⇛⇒→β* a₁⇛a₂)
  ⇛⇒→β* (a₁⇛a₂ !)  = gmap _! _! (⇛⇒→β* a₁⇛a₂)
  ⇛⇒→β* (⟨_⟩! {a₁} {a₂} a₁⇛a₂) = begin
    ⟨ a₁ ⟩ !   ⟶⋆⟨ gmap (λ a → ⟨ a ⟩ !) (λ a₁→a₂ → ⟨ a₁→a₂ ⟩ !) (⇛⇒→β* a₁⇛a₂) ⟩
    ⟨ a₂ ⟩ !   ⟶⟨ ⟨ a₂ ⟩! ⟩
    a₂         ∎
  ...
  -- …-injectivity (with respect to ≡β).
  …-inj : ∀ {n} {a₁ a₂ : Term n} {b₁ b₂ s₁ s₂} →
          a₁ … b₁ ∈ s₁ ≡β a₂ … b₂ ∈ s₂ → a₁ ≡β a₂ × b₁ ≡β b₂ × s₁ ≡ s₂
  …-inj a₁…b₁≡a₂…b₂ =
    let a₁≡a₂ , b₁≡b₂ , s₁≡s₂ = Par.…-inj (≡β⇒≡p a₁…b₁≡a₂…b₂)
    in ≡p⇒≡β a₁≡a₂ , ≡p⇒≡β b₁≡b₂ , s₁≡s₂
